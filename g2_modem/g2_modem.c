...
int main() {
    ...
    g2_modem_program_init(pio, sm, offset, G2_PIN_IN_BASE, G2_PIN_OUT_BASE);

    // The state machine is now running. Any value we push to its TX FIFO will
    // appear on the LED pin.
    while (true) {
        uint32_t action = pio_sm_get_blocking(pio, sm);
        uint32_t data = (action >> 0) & G2_DATA_MASK;
        uint32_t addr = (action >> 8) & G2_ADDR_MASK;
        uint32_t is_read = action & G2_READ_MASK;
